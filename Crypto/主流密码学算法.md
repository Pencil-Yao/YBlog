...
设 已知 二元组![](https://latex.codecogs.com/gif.latex?(g, p))

Alice 生成随机值A，计算![](https://latex.codecogs.com/gif.latex?g^{A}modp=G^{A})
Bob  生成随机值B， 计算![](https://latex.codecogs.com/gif.latex?g^{B}modp=G^{B})

Alice 计算![](https://latex.codecogs.com/gif.latex?S_{A}=(G^{A})^{B}modp)
Bob 计算![](https://latex.codecogs.com/gif.latex?S_{B}=(G^{B})^{A}modp)

我们需要证明![](https://latex.codecogs.com/gif.latex?S_{A}=S_{B})

![](https://latex.codecogs.com/gif.latex?S_{A}=(G^{A})^{B}modp=(g^{A}modp)^{B}modp)

令![](https://latex.codecogs.com/gif.latex?q^{B}modp=T)，则![](https://latex.codecogs.com/gif.latex?q^{B}=kp+T)，也即![](https://latex.codecogs.com/gif.latex?T= q^{B}-kp)

![](https://latex.codecogs.com/gif.latex?S_{A}=(q^{B}modp)^{A}modp=T^{A}modp=(q^{B}-kp)^{A}modp)

由于对 ![](https://latex.codecogs.com/gif.latex?(q^{B}-kp)^{A}) 展开，除了第一项为 ![](https://latex.codecogs.com/gif.latex?q^{AB}) 以及最后一项为![](https://latex.codecogs.com/gif.latex?(kp)^{A})，其余每一项均存在![](https://latex.codecogs.com/gif.latex?p)，所以计算![](https://latex.codecogs.com/gif.latex?(q^{B}-kp)^{A}modp)之后，只保留了第一项，即![](https://latex.codecogs.com/gif.latex?S_{A} = q^{ab}modp)

同理可正![](https://latex.codecogs.com/gif.latex?S_{B}=q^{BA}modp=S_{A})
...
![](https://latex.codecogs.com/gif.latex?n=pq)

随机选取加密秘钥e，使e和(p-1)(q-1)互素。最后用阿几里德扩展算法计算解密秘钥d，以满足

![](https://latex.codecogs.com/gif.latex?ed\equiv1mod(p-1)(q-1))

![](https://latex.codecogs.com/gif.latex?so: d=e^-1mod(p-1)(q-1))

d与n为互素，同样与![](https://latex.codecogs.com/gif.latex?(p-1)(q-1))互素。
加密的过程：

![](https://latex.codecogs.com/gif.latex?c=m^{e}modn)

解密的过程：

![](https://latex.codecogs.com/gif.latex?m=c^{d}modn)

数学证明：

![](https://latex.codecogs.com/gif.latex?m=(m^{e}modn)^{d}modn=m^{e}dmodn=m^{k(p-1)(q-1)+1}modn)

![](https://latex.codecogs.com/gif.latex?=mm^{k(p-1)(q-1)}modn=m)
...
![](https://latex.codecogs.com/gif.latex?y^{2}=x^{3}+ax+b)
...
上述第二种“除法”实际是在被除数上加了若干除数，使得被除数的低位变为0，而高位与除数接近（所以也可以让其比除数小），将高位当做余数。用S记被除数，M记除数，q记累加M的次数，R记2^bitlen，其中bitlen指归0的个数，则余数的表达式为：![](https://latex.codecogs.com/gif.latex?(S+qM)/R)。

![](https://latex.codecogs.com/gif.latex?(S+qM)/R=SR^{-1}modM)
...
![](https://latex.codecogs.com/gif.latex?A'=ARmodM)

![](https://latex.codecogs.com/gif.latex?B'=BRmodM)

![](https://latex.codecogs.com/gif.latex?R=MontPro(A',B')=A'B'=A'B'R^{-1}=ABRmodM)

![](https://latex.codecogs.com/gif.latex?AB=MontPro(R,1))
...
![](https://latex.codecogs.com/gif.latex?P\left(X_{1}, Y_{1}, Z_{1}\right), Q\left(X_{2}, Y_{2}, Z_{2}\right), R\left(X_{3}, Y_{3}, Z_{3}\right), R=P+Q)

的情形，

![](https://latex.codecogs.com/gif.latex?\left\{\begin{array}{l}
u_{1}=X_{1} Z_{2}^{2} \\
u_{2}=X_{2} Z_{1}^{2} \\
s_{1}=Y_{1} Z_{2}^{3} \\
s_{2}=Y_{2} Z_{1}^{3} \\
h=u_{2}-u_{1} \\
r=s_{2}-s_{1} \\
X_{3}=r^{2}-h^{3}-2 u_{1} h^{2} \\
Y_{3}=r\left(u_{1} h^{2}-X_{3}\right)-s_{1} h^{3} \\
Z_{3}=Z_{1} Z_{2} h
\end{array}\right.)
